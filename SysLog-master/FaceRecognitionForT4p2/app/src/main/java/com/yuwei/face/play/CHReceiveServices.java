...
public class CHReceiveServices implements VideoConnStatusCallBack,CameraStatusCallBack,VideoPlayCallBack {
	...
	private CHReceiveServices(){
		...
		new Thread() {
			@Override
			public void run() {
				if(FaceService.getFaceService()!=null && FaceService.getFaceService().isConnected()){
					sendEmptyMessageDelayed(MSG_CONNECT_INIT, 5000);
				} else{
					sendEmptyMessageDelayed(MSG_CONNECT_TIMEOUT, 5000);
					if (mGossClient == null){
						mGossClient = new GossClient();
	 ...
	 * 初始化人脸识别摄像头
	 */
	public void initFaceRecoginzeCamera(Surface surface, ReceiveYuvFrameCallBack yuvFrameCallBa){
		mSurface = surface;
		if(mH264Player != null) {
			mH264Player.setmReceiveYuvFrameCallBack(yuvFrameCallBa);
		}
	}
