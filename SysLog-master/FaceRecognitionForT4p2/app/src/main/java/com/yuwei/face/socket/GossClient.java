...
//			Log.d(TAG,"---->> receive video frame, add data, channel:" + ch );
			try {
				if(CHReceiveServices.getChReceiveServices() != null && gStatus == STATUS_CONNECT)
					CHReceiveServices.getChReceiveServices().AddMediaPackage(mediaData);
				if(FaceService.getFaceService() != null && gStatus == STATUS_CONNECT && CHReceiveServices.getChReceiveServices() == null)
					FaceService.getFaceService().AddMediaPackage(mediaData);
		...
		case 2: //音频
			...
			try {
//				if(CHReceiveServices.getInstance() != null)
...
//			Log.d(TAG, "接收到心跳应答" + String.format("%04x", msgid));
	...
	public void playRealTimeVideo(int channel) {
		...
		Log.i(TAG,"6====>>send to va playRealTimeVideo channel:" + channel);
	...
	public void stopRealTimeVideo() {
		...
		Log.i(TAG,"6====>>send to va stopRealTimeVideo all channel:");
