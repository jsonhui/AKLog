package com.yuwei.face.faceserver;
...
import com.yuwei.face.model.FaceRegisterInfo;
import com.yuwei.face.util.ImageUtil;
import com.yuwei.sdk.certificate.util.FileManager;
...
public class FaceServer {
    ...
    public static final String FEATURE_SUFFIX = ".dat";
  ...
  //复制TF卡中DriverFace文件夹内容到/mnt/sdcard/register
    public int copyDriverRegisterToLocal(String path) {
    	File filesDir = new File(path);
        if (!filesDir.exists() || !filesDir.isDirectory()) {
            return 0;
        }
        File[] registerFiles = filesDir.listFiles();
        if (registerFiles == null || registerFiles.length == 0) {
            return 1;
        }
        (new File(ROOT_PATH + File.separator + SAVE_IMG_DIR)).mkdirs(); // 如果文件夹不存在 则建立新文件夹
        (new File(ROOT_PATH + File.separator + SAVE_FEATURE_DIR)).mkdirs(); // 如果文件夹不存在 则建立新文件夹
        for (File registerFile : registerFiles) {
        	if(registerFile.getName().endsWith(".jpg")) {
        		FileManager.writeFile(registerFile, ROOT_PATH+File.separator+SAVE_IMG_DIR+File.separator+registerFile.getName());
        	} else {
        		FileManager.writeFile(registerFile, ROOT_PATH+File.separator+SAVE_FEATURE_DIR+File.separator+registerFile.getName());
        	}
     ...
     * @param context 上下文对象
     */
    public void resetFaceList(Context context){
    	initFaceList(context);
    }
        ...
        synchronized (this) {
            ...
            for (File featureFile : featureFiles) {
                try {
                    ...
                    fis.read(feature);
                    fis.close();
                    faceRegisterInfoList.add(new FaceRegisterInfo(feature, getName(featureFile.getName())));
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
    ...
    private String getName(String str) {
    	return (str.endsWith(FEATURE_SUFFIX) && str.length()>4) ? str.substring(0, str.length()-4) : str;
    }
    
    public int exportDriverRegisterToTFCard(String filePath){
		synchronized (this) {
            if (ROOT_PATH == null) {
                ROOT_PATH = Environment.getExternalStorageDirectory().getPath();
            }
            Log.d(TAG, "driverInfoToTFCard---ROOT_PATH:"+ROOT_PATH);
            File featureDir = new File(ROOT_PATH + File.separator + SAVE_FEATURE_DIR);
            if (!featureDir.exists() || !featureDir.isDirectory()) {
                return 0;
            }
            File[] featureFiles = featureDir.listFiles();
            Log.i(TAG, "driverInfoToTFCard---featureFiles : " + (featureFiles == null ? null : featureFiles.length));
            if (featureFiles == null || featureFiles.length == 0) {
                return 1;
            }
            (new File(filePath)).mkdirs(); // 如果文件夹不存在 则建立新文件夹
            for (File featureFile : featureFiles) {
                FileManager.writeFile(featureFile, filePath+featureFile.getName());
            }
            ...
            for (File imgFile : imgFiles) {
                FileManager.writeFile(imgFile, filePath+imgFile.getName());
            }
            return 2;
        }
	}
            ...
            if (code == ErrorInfo.MOK && faceInfoList.size() > 0) {
                ...
                try {
                    ...
                    if (code == ErrorInfo.MOK) {
                        ...
                        FileOutputStream fosFeature = new FileOutputStream(featureDir + File.separator + userName + FEATURE_SUFFIX);
